create index if not exists idx_programs_read_model_delta
on public.programs (updated_at desc, created_at desc, id);

create index if not exists idx_program_list_index_delta_freshness
on public.program_list_index (indexed_at, updated_at, id);
