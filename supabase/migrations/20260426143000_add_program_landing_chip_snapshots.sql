  ...
  source_rows as materialized (
    select
      ...
      jsonb_build_object(
        ...
        'link', nullif(btrim(coalesce(p.link, i.link, '')), ''),
        ...
        'application_url', nullif(btrim(coalesce(i.primary_link, p.application_url, p.source_url, '')), ''),
        'application_method', nullif(btrim(coalesce(p.application_method, '')), ''),
...
begin
  ...
  landing_snapshot_result := public.refresh_program_landing_chip_snapshots('landing-c', 24);
  ...
  return jsonb_build_object(
    ...
    'landing_snapshot', landing_snapshot_result
  );
end;
$$;

comment on table public.program_landing_chip_snapshots is
  'Daily landing-c opportunity-feed chip snapshots. Stores precomputed candidate rows per chip so public landing cards stay fixed during the day.';

comment on function public.refresh_program_landing_chip_snapshots(text, integer) is
  'Refreshes landing-c chip snapshots from open program_list_index rows and supporting programs metadata.';
