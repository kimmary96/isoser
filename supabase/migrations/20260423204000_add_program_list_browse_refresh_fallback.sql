create index if not exists idx_program_list_index_browse_refresh_candidates
on public.program_list_index (source, recommended_score desc, updated_at desc, indexed_at desc, id)
where is_open and not coalesce(is_ad, false);
...
select public.refresh_program_list_browse_pool(300);
