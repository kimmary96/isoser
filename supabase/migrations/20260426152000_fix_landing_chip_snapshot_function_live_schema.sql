  ...
  source_rows as materialized (
    select
      ...
      jsonb_build_object(
        ...
        'application_method', application_method_text,
...
exception
  when others then
    raise notice 'initial landing chip snapshot refresh skipped: %', SQLERRM;
end;
$$;

comment on function public.refresh_program_landing_chip_snapshots(text, integer) is
  'Refreshes landing-c chip snapshots from open program_list_index rows and live-compatible canonical/source-record metadata.';
