create or replace function public.program_list_kst_today()
returns date
language sql
stable
...
declare
  ...
  effective_bucket_date date := timezone('Asia/Seoul', effective_viewed_at)::date;
...
begin
  ...
  candidate_ids as materialized (
    ...
    where i.id is null
      ...
      or timezone('Asia/Seoul', i.indexed_at)::date < base_date
    order by
  ...
  ranked as (
    select
      ...
      case
        when s.deadline_confidence_calc = 'high' and s.resolved_deadline is not null and s.resolved_deadline >= base_date and s.resolved_deadline <= base_date + 3 then 1.0
        when s.deadline_confidence_calc = 'high' and s.resolved_deadline is not null and s.resolved_deadline > base_date + 3 and s.resolved_deadline < base_date + 30 then 1.0 - ((s.resolved_deadline - base_date - 3)::numeric / 27.0)
        else 0.0
      end as urgency_calc,
      case
        when coalesce(s.updated_at, s.created_at) is null then 0.3
        when timezone('Asia/Seoul', coalesce(s.updated_at, s.created_at))::date >= base_date - 7 then 1.0
        when timezone('Asia/Seoul', coalesce(s.updated_at, s.created_at))::date <= base_date - 120 then 0.0
        else 1.0 - (((base_date - timezone('Asia/Seoul', coalesce(s.updated_at, s.created_at))::date) - 7)::numeric / 113.0)
  ...
  select
    ...
    parsed_start_date,
    parsed_end_date,
    coalesce(is_active, resolved_deadline is null or resolved_deadline >= base_date),
    is_active,
    ...
    case when resolved_deadline is null then null else resolved_deadline - base_date end,
    null::integer,
...
create or replace function public.refresh_program_list_browse_pool_daily(
...
begin
  loop
    ...
    exit when batch_affected < effective_batch_limit or batches_run >= effective_max_batches;
  end loop;
...
begin
  ...
  perform cron.schedule(
    ...
    '0 15 * * *',
    $job$select public.refresh_program_list_browse_pool_daily(300, 500, 20);$job$
  );
end;
$$;
