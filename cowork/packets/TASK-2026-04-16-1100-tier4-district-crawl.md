...
- 현재 `HEAD` 기준 scheduler에는 Tier 3 collector와 `run_all_collectors(upsert=False)` dry-run 경로가 이미 포함되어 있다
- 기존 Tier 1~3 등록은 유지하고, 6개 Tier 4 collector만 추가 import 및 등록한다
- 실행 순서는 기존 tier 정렬 규칙을 유지한 채 Tier 1 → Tier 2 → Tier 3 → Tier 4가 되도록 한다
...
1. Tier 4 collector 구현 파일을 `tier4_collectors.py`로 분리할지, 기존 collector 모듈에 추가할지 — 현재 `HEAD` 구조와 인접 파일 크기를 보고 최소 변경으로 판단
...
- 자동 복구 기준 재검토 완료: 현재 baseline은 `b994efe8e9ba084b7a73e601bec0a3e7a8b7872f`
- 현재 scheduler baseline에는 Tier 3 등록과 dry-run 경로가 이미 반영되어 있으므로, 재시도 시 해당 동작을 전제로 Tier 4만 추가한다
...
- source_task: `tasks/blocked/TASK-2026-04-16-1100-tier4-district-crawl.md`
- failure_stage: `blocked`
- failure_report: `reports/TASK-2026-04-16-1100-tier4-district-crawl-blocked.md`
