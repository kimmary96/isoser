...
SOURCE = ApiSourceAdapter(
    source_name="work24_job_support",
    display_name="고용24 구직자취업역량 강화프로그램 OpenAPI",
    purpose="구직자 지원 프로그램 목록 수집 및 프로그램 메타데이터 정규화",
    key_env_name="WORK24_JOB_SUPPORT_AUTH_KEY",
    auth_param_name="authKey",
    guide_url_env_name="WORK24_OPEN_API_GUIDE_URL",
)

SEED_DIR = Path(__file__).resolve().parents[1] / "seed_data"
DEFAULT_SAMPLE_DIR = SEED_DIR / "api_samples" / "work24_job_support"

# TODO: Confirm the exact Work24 job-support endpoint and request parameter names.
...
def _extract_records(value: Any) -> list[dict[str, Any]]:
    ...
    if any(key in value for key in ("TRPR_ID", "TITLE", "ADDRESS")):
        return [value]
...
class Work24JobSupportAdapter:
    """Collect Work24 job-support program rows with the shared adapter pattern."""
    ...
    def _resolve_auth_key(self) -> str | None:
        ...
        if not api_key:
            print(f"[work24_job_support] {SOURCE.key_env_name} is not configured.")
            return None
        return api_key
    ...
    def _request_payload(
        ...
        *,
        ...
        if not self.list_endpoint:
            print("[work24_job_support] list endpoint is not configured. TODO: set the exact API endpoint.")
            return None

        last_error: Exception | None = None
        # TODO: Confirm whether these parameter names match the real API.
        params = {
            "returnType": "JSON",
            "outType": "1",
            "pageNum": page_num,
        ...
        print(f"[work24_job_support] {sample_name} request failed: {last_error}")
        return None
    ...
    def fetch_list(
        ...
        try:
            payload = self._request_payload(
                ...
                area_code=area_code,
                ncs_code=ncs_code,
                sample_name=f"job_support_list_page_{page_num}",
            )
            if payload is None:
                return None
            return [self._normalize_program(row) for row in _extract_records(payload)]
        except Exception as exc:
            print(f"[work24_job_support] fetch_list failed: {exc}")
            return None
    ...
    def fetch_all(
        ...
        try:
            first_page_payload = self._request_payload(
                ...
                area_code=area_code,
                ncs_code=ncs_code,
                sample_name="job_support_list_page_1",
            )
            ...
            print(f"[work24_job_support] fetch_all failed: {exc}")
            return None


def main() -> None:
    adapter = Work24JobSupportAdapter()
    rows = adapter.fetch_all()
    print(f"[work24_job_support] rows: {len(rows or [])}")
