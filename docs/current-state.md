...
- 비교 페이지는 `POST /programs/batch`로 기본 프로그램을 한 번에 조회하고, `POST /programs/details/batch` 상세 응답을 함께 붙여 기본 정보, 일정, 비용·지원, 대상·모집, 소개 섹션을 비교한다. 상세 batch 호출이 실패해도 기존 목록 데이터 fallback으로 화면은 유지된다. 비교 프로그램 선택 모달의 찜한 프로그램 탭은 열릴 때마다 `program_bookmarks` 기반 dashboard bookmark BFF를 재조회해 최신 북마크를 보여주고, 전체 검색 탭은 `/programs/filter-options`를 재사용해 운영 기관, 추천 대상, 선발 절차, 채용 연계 옵션을 실제 프로그램 데이터 기반으로 표시한다. 선택된 옵션은 기존 `/programs` 목록 query 파라미터로 검색 결과에 적용한다.
