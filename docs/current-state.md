...
- `GET /programs/list` now returns `promoted_items` separately from organic `items`. The read-model organic path adds `is_ad=false`, first-page browse requests fetch explicit `is_ad=true` promoted rows first, then use configurable `PROGRAM_PROMOTED_PROVIDER_MATCHES` (default Fast Campus/패스트캠퍼스 demo assumption) as a sponsored fallback without duplicating the same program in organic results. The frontend renders these rows in a separate `스폰서 추천` layer above the organic table.
...
- `scripts/html_collector_diagnostic.py --include-ocr-probe`는 OCR 런타임 없이 HTML collector detail page를 source별 소량 샘플링해 이미지/첨부 중심 공고 후보를 읽기 전용으로 분류한다. Markdown/JSON 리포트는 poster/attachment 검토 후보, detail/parser 보강 우선 source, sample detail 상태뿐 아니라 source별 attachment/image URL sample도 함께 보여준다. 2026-04-24 live dynamic 진단에서는 HTML collector 14개 중 즉시 Playwright 후보 0개, partial parse-empty 모니터링 source 2개, snapshot 2건, aggregated dry-run quality `checked_rows=187`이 확인됐다. 같은 날 OCR preflight는 live timeout 1건 때문에 `detail_probe_inconclusive`가 4개로 늘었지만, 즉시 OCR runtime opt-in 후보는 여전히 0개였다.
