...
- `frontend/app/(landing)/programs/page.tsx`는 URL query 기반 검색, 카테고리/세부 카테고리/지역/수업 방식/비용/참여 시간 필터, 모집중 토글, 정렬, 페이지네이션을 지원한다. 상단 필터 UI는 `frontend/app/(landing)/programs/programs-filter-bar.tsx`로 분리되어 카테고리, 온/오프라인, 시·도 지역 다중 선택, 비용 다중 선택, 참여 시간 다중 선택, 운영 기관, 추천 대상, 정렬, 검색 input, 검색 버튼, 초기화, 추가 필터, 활성 필터 chip을 렌더링한다. 목록 필터에서 선발 절차와 채용 연계는 제거됐고, 검색 input/검색/초기화는 필터 버튼 아래 별도 줄에 배치된다. 다중 선택 메뉴는 항목 클릭 즉시 선택 상태를 바꾸고 메뉴를 닫으며, 정렬은 select 변경 즉시 URL query에 반영된다. 세부 카테고리는 `category_detail` query와 backend `/programs` API로 전달되며, 운영 DB에 `programs.category_detail` migration이 아직 적용되지 않은 환경에서는 backend가 큰 카테고리 필터로 fallback한다.
...
- 프로그램 목록 페이지는 상단 필터, 1줄 가로 스크롤 마감 임박 레일, 엑셀형 전체 프로그램 테이블로 구성된다. 맞춤 추천 섹션은 목록 화면에서 제거되어 전체 프로그램 비교 탐색을 우선한다. `/programs` 본문 컨테이너는 `max-w-[1680px]` 기준으로 Program Search, Closing Soon, 전체 프로그램 카드가 같은 좌우 폭에 정렬된다. Closing Soon은 검색/필터 조건과 별개로 전역 마감임박 목록을 조회하는 한 행 가로 레일이며, 초과 카드는 오른쪽으로 이어지고 레일 내부에서 가로 스크롤된다. 이 레일은 일반 `ProgramCard` 대신 마감일, 제목, 핵심 chip, 2줄 설명만 표시하는 압축 카드로 렌더링해 필터와 전체 목록 사이의 세로 간격을 줄인다. 전체 목록 테이블은 제목/source가 있는 프로그램을 노출하며, 모집 마감일이 resolved 되지 않은 row는 `마감일 미확인`으로 표시한다. 제목 링크는 상세 이동, 테이블과 카드형 프로그램 UI의 별 버튼은 공용 `ProgramBookmarkButton`을 통해 dashboard bookmark BFF 찜 mutation을 수행한다. 비로그인 사용자가 별 버튼을 누르면 버튼을 비활성화하지 않고 현재 `/programs` 경로와 query/hash를 `redirectedFrom`으로 담아 로그인 페이지로 이동한다. 테이블의 프로그램 과정은 backend `display_categories`를 우선 사용해 대표 카테고리 1~2개를 badge로 표시하고, 참여 시간은 `participation_mode_label`과 `participation_time_text`를 함께 보여주며, 선발절차·키워드 칸은 `selection_process_label`과 `extracted_keywords` chip을 표시한다. 전체 프로그램 테이블에서는 채용연계 열을 표시하지 않고, 해당 원본 데이터는 비교/상세 등 다른 화면에서 재사용할 수 있도록 API 필드에는 유지한다. 테이블은 내부/카드 가로 스크롤 없이 전체 카드 폭 안에서 고정 레이아웃으로 펼쳐지고, 긴 제목과 키워드는 줄바꿈으로 수용한다. 파생 키워드가 없으면 chip 블록만 숨기고, 선발 절차가 없으면 중복 dash 없이 `선발 절차 없음`만 표시한다. 로그인 사용자는 서버 렌더링 시 `program_bookmarks`에서 기존 찜 program id를 읽어 초기 별 상태에 반영하며, `/programs` 화면 안에서는 같은 program id 카드/행이 공유 bookmark state를 사용해 찜 변경을 즉시 동기화한다. 필터는 지역, 비용, 참여 시간, 운영 기관, 추천 대상 조건을 URL query와 backend 목록/count API에 연결한다. 다중 필터는 항목 클릭 후 메뉴가 자동으로 닫히고, 정렬은 변경 즉시 적용된다. 운영 기관과 추천 대상 옵션은 `GET /programs/filter-options`가 현재 검색/카테고리/지역/수업방식/모집 상태 조건에 맞는 실제 프로그램 데이터에서 추출하며, 옵션 조회 실패나 빈 결과에서는 기존 정적 옵션으로 fallback한다.
