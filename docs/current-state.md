...
- `BaseHtmlCollector.collect_url_items()`는 Tier 2/3/4 HTML collector의 URL별 request failure, parse-empty, parse failure를 구조화된 `last_collect_url_diagnostics`와 공통 `last_collect_message`로 남긴다. `scripts/html_collector_diagnostic.py --include-scheduler-summary --snapshot-output-dir <dir>`는 이 신호와 normalize 결과를 같은 JSON/Markdown 리포트에 묶고, partial parse-empty URL의 bounded HTML snapshot을 별도 파일로 저장한다. 2026-04-23 live 진단에서는 HTML collector 14개 중 즉시 Playwright 후보는 0개이고, 도봉구청 일자리경제과/서울청년센터 성동만 partial parse-empty 모니터링 대상으로 분류됐으며 snapshot 2건과 HTML source 기준 dry-run quality summary(`checked_rows=190`, `info=178`)를 함께 남겼다.
- `scripts/html_collector_diagnostic.py --include-ocr-probe`는 OCR 런타임 없이 HTML collector detail page를 source별 소량 샘플링해 이미지/첨부 중심 공고 후보를 읽기 전용으로 분류한다. Markdown 리포트는 poster/attachment 검토 후보, detail/parser 보강 우선 source, sample detail 상태를 함께 보여준다. 2026-04-23 live preflight에서는 OCR runtime opt-in 후보 0개, poster/attachment 검토 후보 7개, detail/parser 보강 우선 3개로 분리됐다.
