...
- 비교 페이지는 목록 API의 `Program`에 `POST /programs/details/batch` 상세 응답을 함께 붙여 기본 정보, 일정, 비용·지원, 대상·모집, 소개 섹션을 비교한다. 상세 batch 호출이 실패해도 기존 목록 데이터 fallback으로 화면은 유지된다.
...
- 고용24/K-Startup 프로그램 매핑은 제목, 설명, 대상, 사업 유형, 고용24 NCS 코드에서 보수적인 스킬 키워드를 추출해 `programs.skills` 배열로 넘기고, normalizer는 이를 중복 제거된 text list로 저장 payload에 포함한다.
- compare relevance 응답은 기존 점수 필드 외에 `region_match_score`, `matched_regions`, `fit_label`, `fit_summary`, `readiness_label`, `gap_tags`를 함께 반환한다. 프로필 지역 정보가 없으면 기존 관련도 점수를 유지하고, 지역 정보가 있으면 프로그램 지역/위치 텍스트와의 보수적 일치 신호를 최대 15% 반영한다. compare UI는 이를 `★ AI 적합도` 섹션에서 적합도 판단, 지원 준비도, 한줄 요약, 보완 포인트로 해석해 보여준다.
