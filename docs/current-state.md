...
- `frontend/app/(landing)/programs/page.tsx`는 URL query 기반 검색, 카테고리/세부 카테고리/지역/수업 방식/비용/참여 시간 필터, 모집중 토글, 정렬, 페이지네이션을 지원한다. 상단 필터 UI는 `frontend/app/(landing)/programs/programs-filter-bar.tsx`로 분리되어 검색 input, 카테고리, 온/오프라인, 시·도 지역 다중 선택, 비용 다중 선택, 참여 시간 다중 선택, 정렬, 추가 필터, 초기화, 활성 필터 chip을 렌더링한다. 세부 카테고리는 `category_detail` query와 backend `/programs` API로 전달되며, 운영 DB에 `programs.category_detail` migration이 아직 적용되지 않은 환경에서는 backend가 큰 카테고리 필터로 fallback한다.
...
- `frontend/app/(landing)/programs/[id]/page.tsx`는 상세 전용 API를 사용해 서버에서 데이터를 가져오고, `program-detail-client.tsx`가 Hero, 탭, 본문/사이드바 2열, 빠른 목차, 북마크/공유 UI를 렌더링한다. 값이 없는 provider/location/description/선택 운영 정보는 한 줄씩 `정보 없음`으로 표시하지 않고 해당 chip 또는 섹션을 숨긴다.
...
- 고용24와 K-Startup Tier 1 collector는 API raw에 포함된 기관명, 지역, 설명, 시작/종료일, 원본 링크, 주요 추적 메타를 공통 normalizer row까지 보존한다. 상세/비교 화면이 이미 참조하는 `provider`, `location`, `description`, `start_date`, `end_date`, `source_url`은 값이 있을 때 저장 payload에 포함되며, source별 부가 추적값은 기존 `compare_meta` JSONB에 보존한다. 고용24의 `traEndDate`는 훈련 종료일이므로 `deadline`으로 저장하지 않고 `end_date` 및 `compare_meta.training_end_date`로만 보존한다. 고용24/K-Startup mapping은 제목, 설명, 대상, NCS 코드에서 보수적인 키워드를 추출해 `skills` 후보를 채우며, normalizer는 중복 없는 문자열 배열로 정리한다.
...
- 비교 페이지는 `POST /programs/batch`로 기본 프로그램을 한 번에 조회하고, `POST /programs/details/batch` 상세 응답을 함께 붙여 기본 정보, 일정, 비용·지원, 대상·모집, 소개 섹션을 비교한다. 상세 batch 호출이 실패해도 기존 목록 데이터 fallback으로 화면은 유지된다.
...
- 프로그램 목록 페이지는 상단 필터, 1줄 가로 스크롤 마감 임박 레일, 엑셀형 전체 프로그램 테이블로 구성된다. 맞춤 추천 섹션은 목록 화면에서 제거되어 전체 프로그램 비교 탐색을 우선한다. 전체 목록 테이블은 제목/마감일/source가 있는 프로그램만 노출하고, 제목 링크는 상세 이동, 별 버튼은 dashboard bookmark BFF를 경유해 찜 mutation을 수행한다. 로그인 사용자는 서버 렌더링 시 `program_bookmarks`에서 기존 찜 program id를 읽어 초기 별 상태에 반영하며, `/programs` 화면 안에서는 같은 program id 카드/행이 공유 bookmark state를 사용해 찜 변경을 즉시 동기화한다. 필터는 지역, 비용, 참여 시간, 운영 기관, 추천 대상, 선발 절차, 채용 연계 조건을 URL query와 backend 목록/count API에 연결한다.
