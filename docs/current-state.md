...
- `scripts/html_collector_diagnostic.py --include-ocr-probe`는 OCR 런타임 없이 HTML collector detail page를 source별 소량 샘플링해 이미지/첨부 중심 공고 후보를 읽기 전용으로 분류한다. Markdown/JSON 리포트는 poster/attachment 검토 후보, detail/parser 보강 우선 source, sample detail 상태뿐 아니라 source별 attachment/image URL sample과 normalized row field gap audit(`field_gap_summary`, `field_gap_audit`)도 함께 보여준다. 2026-04-24 live dynamic 진단에서는 HTML collector 14개 중 즉시 Playwright 후보 0개, partial parse-empty 모니터링 source 2개, snapshot 2건, aggregated dry-run quality `checked_rows=187`이 확인됐다. 같은 날 OCR preflight 재실행에서는 `poster_or_attachment_candidate=7`, `detail_probe_inconclusive=3`, `text_sufficient_no_ocr=4`였고, field gap이 잡힌 13개 source의 이슈는 모두 info-level `missing_provider`(175 rows)라 즉시 OCR runtime opt-in 후보는 여전히 0개였다.
