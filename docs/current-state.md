...
- `frontend/app/(landing)/landing-a/_components.tsx`는 기존 import 호환을 위한 export 허브이며, 실제 섹션 구현은 `_navigation.tsx`, `_hero.tsx`, `_program-feed.tsx`, `_support-sections.tsx`, `_style-tag.tsx`, 공통 유틸/인증 hook은 `_shared.ts`, `_auth.ts`로 분리되어 있다. `_program-feed.tsx`는 프로그램 카드 렌더를 `ProgramCard`로 분리하고, `_navigation.tsx`는 브랜드/프로필 액션/랜딩 A 헤더 링크 패턴을 공유 컴포넌트와 링크 배열로 관리한다.
