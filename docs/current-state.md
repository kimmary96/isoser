...
- `backend/routers/programs.py`는 `/programs/count`와 확장된 목록 query(`q`, `regions`, `teaching_methods`, `cost_types`, `participation_times`, `recruiting_only`, `include_closed_recent`, `sort`)를 지원하고, 목록/카운트 모두 Supabase의 `is_active` 값만 신뢰하지 않고 실제 `deadline` 기준으로 오늘 이후 모집중 공고만 기본 노출한다. `cost_types`는 `cost_type` 컬럼이 있으면 우선 사용하고, 없으면 `cost`, `support_type`, `compare_meta`, 제목/설명 텍스트에서 `내일배움카드`, `무료(내배카 X)`, `유료`를 보수적으로 분류한다. `participation_times`는 `participation_time` 컬럼이 있으면 우선 사용하고, 없으면 제목/설명 키워드와 `start_date`/`end_date` 기간으로 `파트타임`, `풀타임`을 분류한다. 목록 응답은 실제 row의 제목, 요약, 설명, tags, skills, teaching/support 필드, `compare_meta`를 기반으로 `display_categories`, `participation_mode_label`, `participation_time_text`, `selection_process_label`, `extracted_keywords` 파생 표시 필드를 함께 내려준다. 짧은 영문 키워드(`AI`, `DB`, `BI`, `UI` 등)는 원본 URL/식별자와 우연히 매칭되지 않도록 영문 단어 경계가 맞는 경우에만 인정한다. 카드/추천용 D-day와 `days_left`는 모집 마감일(`close_date` 또는 `deadline`)만 사용하며, 고용24에서 `deadline`이 훈련 종료일(`end_date`)과 같은 값으로 저장된 경우에는 D-day 계산에서 제외한다. `recruiting_only=true` 목록/카운트는 이 보정 후 `days_left >= 0`인 항목만 남기며, `sort=latest`는 최신 원천 데이터 첫 묶음에 표시 불가 항목이 몰려도 결과가 비지 않도록 1,000건 단위 후보 scan을 사용한다. `q` 검색은 Supabase `programs.search_text`가 있으면 해당 검색용 컬럼으로 DB 후보를 먼저 줄이고, 백엔드에서 `title > provider > category/category_detail alias > description/summary > location/region > tags/skills > compare_meta` 순서로 공백 제거/lower 기반 부분 검색 및 정렬을 수행한다. `search_text` 컬럼이 없는 환경에서는 기존 1,000건 단위 후보 scan으로 자동 fallback한다.
