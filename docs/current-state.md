...
- 프로그램 목록 페이지는 맞춤 추천, 마감 임박, 전체 프로그램 섹션으로 나뉘며, 비로그인 사용자는 맞춤 추천 섹션을 블러 처리된 공개 프로그램 카드와 로그인 CTA로 본다. 목록 카드는 제목/마감일/source가 있는 프로그램만 노출하고, 본문 클릭은 상세 이동, 우측 상단 별 버튼은 dashboard bookmark BFF를 경유해 찜 mutation을 수행한다. 로그인 사용자는 서버 렌더링 시 `program_bookmarks`에서 기존 찜 program id를 읽어 카드 초기 별 상태에 반영하며, `/programs` 화면 안에서는 같은 program id 카드들이 공유 bookmark state를 사용해 한 카드의 찜 변경이 추천/마감임박/전체 목록에 즉시 동기화된다. 필터는 지역, 비용, 참여 시간, 운영 기관, 추천 대상, 선발 절차, 채용 연계 조건을 URL query와 backend 목록/count API에 연결한다.
