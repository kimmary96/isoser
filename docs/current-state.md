...
- `frontend/app/(landing)/landing-c/page.tsx`와 `landing-c/_program-utils.ts`는 랜딩 `Live Board`를 특정 source quota 대신 이번 주 내 마감하는 모집중 공고 상위 3건으로 고르고, `Opportunity feed`는 기본 `전체` 칩에서 모집중이면서 만족도/리뷰/추천 점수가 높은 순으로, `마감임박`과 다른 칩에서는 마감 임박순을 우선 적용한다. 프로그램별 클릭수 집계가 아직 read-model에 없어서 "핫한 공고"는 현재 저장된 만족도/리뷰/추천 점수 proxy를 사용한다.
