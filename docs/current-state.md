...
- `BaseHtmlCollector.collect_url_items()`는 Tier 2/3/4 HTML collector의 URL별 request failure, parse-empty, parse failure를 공통 `last_collect_message`로 남긴다. `scripts/html_collector_diagnostic.py`는 이 신호와 normalize 결과를 읽기 전용으로 분류해 dynamic retrieve/Playwright probe 후보를 JSON/Markdown 리포트로 저장한다. 2026-04-23 live 진단에서는 HTML collector 14개 중 즉시 Playwright 후보는 0개이고, 도봉구청 일자리경제과/서울청년센터 성동만 partial parse-empty 모니터링 대상으로 분류됐다.
- `scripts/html_collector_diagnostic.py --include-ocr-probe`는 OCR 런타임 없이 HTML collector detail page를 source별 소량 샘플링해 이미지/첨부 중심 공고 후보를 읽기 전용으로 분류한다. 2026-04-23 live preflight에서는 OCR runtime opt-in 후보 0개, poster/attachment 검토 후보 7개, detail/parser 보강 우선 3개로 분리됐다.
...
- Tier 4 district collector는 URL별 요청 성공 수, request failure 수, parse-empty 수를 `last_collect_message`에 남기고, scheduler dry-run source message는 raw item 수, dedupe 후 row 수, collector 진단 메시지를 함께 반환한다.
