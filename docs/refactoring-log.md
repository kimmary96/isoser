...
- 2026-04-27: `frontend/app/dashboard/page.tsx`, `docs/current-state.md`
  - 대시보드 전체 색감을 로그인 페이지와 맞추는 시안으로 조정함. 페이지 배경을 연한 블루-화이트 그라데이션으로 바꾸고, 본문을 큰 글래스 패널 안에 두며, 왼쪽 사이드바/툴바/달력/하단 카드 스트립과 추천 카드의 그림자·반투명 흰 패널·블루 계열 버튼 톤을 통일함
- 2026-04-27: `frontend/app/dashboard/page.tsx`, `docs/current-state.md`
  - 대시보드 하단 추천/찜 카드에서 잘린 제목을 바로 확인할 수 있도록 `미리보기` 버튼 기반 과정 상세 모달을 추가함. 모달은 기존 backend 프로그램 상세 조회를 재사용하고, 상세 페이지 보기/신청 링크 열기로 이어지게 연결함
- 2026-04-27: `frontend/app/dashboard/page.tsx`
  - 대시보드 하단 `내 커리어 핏 추천`/`찜한 과정` 카드에서 제목 글자 하단이 잘려 보이던 문제를 줄이기 위해 카드 높이를 132px에서 140px로 늘리고, 제목 2줄 clamp의 줄간격과 하단 여백을 완화함
