# 이소서 (Isoser) v3 — 통합 사업계획서

> "흩어진 국비·지역 지원 프로그램을 한곳에 모으고, 내 이력과 활동 기반으로 맞춤 추천과 빠른 지원 준비를 연결하는 서비스." — PRD v3
> 
...
**이소서 = 국가 취업 지원 프로그램 허브 + 개인화 추천 + 지원 준비 워크플로우**
HRD클럽([hrdclub.co.kr](http://hrdclub.co.kr))이 하던 국비 교육 정보 + 훈련기관 광고 플랫폼을, 인프런([inflearn.com](http://inflearn.com)) 수준의 UI/UX로 재구축하고, 사용자의 이력·활동 데이터를 기반으로 개인화 추천과 지원 준비를 연결하는 서비스.
...
**이소서는 이력서 편집기가 아닙니다.**
이력서 기능은 본체가 아니라 추천 정확도와 지원 속도를 높이는 연결 장치입니다.
경쟁 상대는 AI 자소서 서비스가 아니라, 고용24, 지자체 사이트, HRD클럽처럼 **기회 탐색은 되지만 개인화와 지원 연결이 약한 정보 플랫폼**입니다.
...
| **기존 방식** | **이소서 v2** |
| --- | --- |
| 고용24, 지자체, 커뮤니티를 각각 뒤져야 함 | 내 이력/활동 기반으로 맞춤 추천 |
| 마감일 직접 추적 | 캘린더에 자동 노출, 마감 임박 하이라이트 |
| 프로그램 지원 시 서류를 다시 준비해야 함 | 저장된 활동 데이터로 빠르게 지원 서류 생성 |
| 정부 사이트의 낙후된 UX | 인프런 수준의 깔끔한 대시보드 |
| 프로그램 참여 후 남는 기록이 흩어짐 | 활동/성과를 다시 저장해 다음 추천과 취업 준비에 재사용 |

### 1-5. v1 → v2 진화 스토리

| **구분** | **v1 (메인프로젝트 1)** | **v2 (메인프로젝트 2)** |
| --- | --- | --- |
| 핵심 | AI 코치 기반 이력서 편집·관리 | v1 + **국가 취업 지원 프로그램 허브** |
...
**왜 피벗했나?** v1은 AI 코치 기반 이력서 편집 서비스였지만, **"굳이 이소서를 써야 할 이유"**가 약했다. 문장 다듬기는 ChatGPT로도 가능하니까. 그래서 이소서는 이력서 기능을 전면 서비스가 아니라 **기회 탐색과 지원 전환을 완성하는 장치**로 재배치했다. 정보 허브를 결합하면 (1) 다른 곳에 없는 국비·지역 프로그램 정보로 유저를 데려오고, (2) 더 정확한 추천을 위해 이력/활동 데이터를 쌓게 만들고, (3) 지원 시 필요한 문서를 빠르게 만들게 하며, (4) 프로그램 참여 경험이 다시 성과 데이터로 돌아오는 순환 구조가 생긴다.
...
**1단계 — 유입**: 무료 취업 정보 허브로 유저를 데려온다. 핵심은 다른 곳에 흩어진 국비·지역 프로그램을 한곳에 모아 보여주는 것이다.

**2단계 — 개인화**: 단순 목록 서비스와 달리, 사용자의 이력·활동을 등록하면 더 정밀한 추천이 가능해진다. 이력/활동 등록이 추천 품질을 높이는 핵심 입력이 된다.

**3단계 — 지원 전환**: 프로그램에 지원하려는 순간 이력서·포트폴리오·자소서가 필요해진다. 이소서는 이미 등록된 데이터를 바탕으로 빠른 지원 준비를 제공한다.

**4단계 — Lock-in**: 프로그램 참여 후의 경험과 성과가 다시 저장소에 쌓이고, 이후 다른 프로그램 추천과 취업 지원에 재사용된다.

**5단계 — 수익화**: 훈련기관 광고 상품(핵심 BM) + 구글 애드센스(초기) + 광고 제거 구독(알라미 모델) + 장기적으로 고도화된 지원 준비 기능 확장.
...
| **단계** | **v1 기능의 역할** | **목적** |
| --- | --- | --- |
| **지금** | 추천 정확도와 지원 속도를 높이는 연결 장치 | 정보 허브를 단순 목록이 아니라 행동 플랫폼으로 만듦 |
| **유저 확보 후** | Lock-in 도구 | 프로그램 참여 성과가 쌓여 다음 추천과 취업 준비에 재사용 |
| **장기** | 고도화 → 별도 유료 서비스 | 고급 추천, 지원 전략, 포트폴리오/서류 고도화 등 추가 수익원 |
...
flowchart TB
    ...
    subgraph S3["🔒 3. Lock-in"]
        H["성과 데이터 누적<br>= 다음 추천/지원의 입력 데이터<br>→ 이관 비용 증가"]
    end
    subgraph S4["💰 4. 수익화"]
        I["초기: 구글 애드센스"]
...
- **v3.0** (2026-04-20): 제품 정체성을 `프로그램 허브 + 개인화 추천 + 지원 준비 워크플로우` 중심으로 재정의하고, 순환 플로우와 핵심 메시지를 그 방향에 맞게 개정.
