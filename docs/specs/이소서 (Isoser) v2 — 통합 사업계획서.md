...
> "파편화된 국가 취업 지원 사업 정보를 AI가 나에게 맞게 골라서 캘린더로 보여주고,
지원에 필요한 서류를 성과저장소 데이터로 즉시 만들어주는 서비스." — PRD v2.1
> 
...
| **구분** | **특징** | **핵심 니즈** | **v2 해결** |
| --- | --- | --- | --- |
| **A — 부트캠프 취준생** | 프로젝트 3~5개, 여러 직무 동시 지원 | 프로젝트별 정리 + 이력서·포폴에 녹이기 | 성과저장소 + AI 코치 + 포트폴리오 자동 생성 |
...
### 7-1. 🗓️ AI 맞춤 캘린더 (신규, 메인프로젝트 2)

- 사용자 프로필 기반 **개인화 추천** (HRD넷·고용24·SBA 수집 데이터)
...
- 추가: 관심 프로그램 북마크, 지원 상태 트래킹
- 추천 결과 `recommendations` 테이블에 24시간 캐싱
...
- **포트폴리오 자동 생성** — 6섹션 (프로젝트 개요 / 문제 정의 / 기술적 의사결정 / 구현 / 성과 / 트러블슈팅)
...
| 하이브리드 리트리버 | Sparse(직무명, 지역) + Dense(의미 유사도) | `routers/recommend.py` |
| 리랭킹 | 관련도(60%) + 마감 임박도(40%) 합산 | `chains/` |
| LLM | Gemini 2.5 Flash (추천+챗봇) / 2.0 Flash (프론트 요약) | FastAPI / `app/api/summary` |
| 개인화 | 성과저장소 직무 키워드·기술 태그 → 프로필 임베딩 동적 업데이트 | `routers/recommend.py` |
...
## 11. 구현 현황 (메인프로젝트 1 완료)

<aside>
✅

v1에서 **이미 구현 완료**된 기능. v2에서 재활용 + 확장.

</aside>

- **인증**: Google OAuth + Supabase 세션 + 게스트 모드
...
| 출처 | 수집 대상 | 방식 |
| --- | --- | --- |
| **HRD넷** ([hrd.go.kr](http://hrd.go.kr)) | 국비 훈련 과정, 일정, 모집 인원 | 공공 API |
| **고용24** ([work.go.kr](http://work.go.kr)) | 청년 취업 지원 사업, 국비 인턴 | 공공 API |
| **K-디지털 훈련** | 부트캠프 과정 목록 | 공공 API |
...
| **HRD클럽** ([hrdclub.co.kr](http://hrdclub.co.kr)) | 훈련기관 광고 데이터 | 낮음 |
| **각 지역 고용센터** | 지역별 취업 지원 행사 | 높음 |

<aside>
⚠️

고용24, HRD넷 일부는 크롤링 차단 가능. API 제공 여부 먼저 확인. 차단 시 공공데이터포털 Open API 또는 RSS 대체.
**최소 기준**: HRD넷 + 고용24 API만으로 MVP 가능.
...
| POST | `/programs/sync` | 데이터 동기화 (관리자) | `rag/collector/` |
| POST | `/recommend` | 맞춤 추천 (RAG) | `routers/recommend.py` |
| GET | `/recommend/calendar` | 캘린더용 추천 | `routers/recommend.py` |
| POST | `/chat` | 정보 검색 AI 챗봇 (RAG) | `routers/recommend.py` |
| POST | `/portfolio/generate` | 포트폴리오 자동 생성 | `routers/portfolio.py` |
| POST/GET | `/bookmarks` | 북마크 저장/목록 조회 | `routers/programs.py` |
...
- `/` 또는 `/programs`: 국가 취업 지원 프로그램 전체 목록 (인프런 스타일)
- `/programs/[id]`: 프로그램 상세 + 이력서 즉시 생성 버튼
- `/dashboard/portfolio`: 포트폴리오 자동 생성 및 편집
- `/chat`: 정보 검색 AI 챗봇
...
- **캐싱**: 추천 결과 24시간 캐싱, 프로그램 데이터 주 1회 이상 동기화
...
- `20260410_create_programs.sql`
- `20260410_create_recommendations.sql`
- `20260410_create_bookmarks.sql`
...
| 기간 | 마일스톤 | 주요 작업 | 산출물 |
| --- | --- | --- | --- |
| **D1~D3** | 데이터 수집 인프라 | 공공 API 연동, 데이터 정제 파이프라인, RAG DB 초기 적재 | `rag/collector/` |
| **D4~D6** | RAG 엔진 핵심 | 프로필/프로그램 임베딩, 하이브리드 검색, 리랭킹. Coach AI Hybrid Search | `rag/embedder.py`, `routers/recommend.py` |
| **D7~D9** | 프론트엔드 MVP | 랜딩 개편 (광고 허브), 대시보드 캘린더 뷰, `/programs` | `/dashboard`, `/programs` |
| **D10~D11** | 통합 및 서류 생성 | 추천 → 서류 생성 순환 플로우, 포트폴리오(6섹션), AI 챗봇 | `routers/portfolio.py` |
| **D12~D13** | 테스트 및 발표 준비 | 테스트 10가지 검증, 라이브 데모 준비, 발표 자료 제작 | 슬라이드, 데모 |
...
- **대응**: 공공 API 우선. **최소**: HRD넷+고용24만으로 MVP 가능
