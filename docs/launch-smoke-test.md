...
- `/` 접속 시 `/landing-c`로 정상 이동하는지 확인
- `/landing-c` 상단 헤더의 `프로그램 상세`, `비교`, `대시보드`, 로그인/프로필 링크 동작 확인
- `/landing-c`에서 Live Board, 검색/칩 필터, Opportunity feed 카드, `과정 보기`/`비교` 액션이 첫 화면과 목록 영역에 정상 렌더링되는지 확인
...
- 기존 사용자 로그인 후 기본 진입이 `/landing-c` 또는 원래 `next` 경로인지 확인
