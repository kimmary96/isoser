...
- 추천 BFF가 성공 응답으로 빈 추천을 반환하거나 마감 제외 후 0건이 되면 모집중 공개 후보 fallback을 사용하도록 복구했다.
...
- 한 번 추천이 로딩되면 재진입 시 빠르게 보이도록 추천 localStorage 캐시를 `isoser:dashboard-recommended-programs:v2` 6시간 TTL로 바꾸고, 모집중이며 양수 점수가 있는 추천/fallback 카드를 즉시 표시하도록 했다.
- 공개 fallback 카드에 추천 점수가 없을 때 0%로 남지 않도록 기존 점수 또는 마감 임박도에서 양수 표시 점수를 만든다.
- 후속 기준 변경으로 `내 커리어 핏 추천`은 70점 threshold를 쓰지 않고 사용자 DB 기반 개인화 추천 surface만 노출한다. 내부 매칭 점수는 정렬에만 사용하고, 카드의 `커리어 핏 N%` 텍스트는 추천/캘린더 카드에서 제거했다.
...
- 커리어 핏 추천 strip이 캘린더/마감 fallback 데이터와 섞여 0% 카드가 먼저 보일 수 있어, 추천 전용 BFF와 커리어 핏 점수 정렬을 사용하도록 맞췄다.

## preserved behaviors

- 대시보드의 큰 월간 캘린더, 날짜 선택, 일정 유형 필터, 찜 표시, 캘린더 담기 동작은 유지했다.
- 추천/찜 카드의 기존 상세 이동, 비교 이동, `/programs` 탐색 링크는 유지했다.
- 추천 localStorage 캐시는 6시간 TTL의 v2 key로 유지하며, 이전 캘린더 fallback 0% cache가 추천 strip을 덮지 않게 분리했다.
- 추천 캐시는 기존 key를 새 v2 key로 분리해 오래된 15분 캘린더 캐시와 섞이지 않는다.
...
- `score/final_score` fallback은 추천 기본 목록에서도 0% 대신 정렬 점수를 보여준다. 다만 프로필 기반 관련도가 전혀 계산되지 않은 fallback 데이터는 순수 개인화 관련도라기보다 추천 정렬 점수에 가까울 수 있다.
- deadline/days_left가 없고 `is_open/is_active`도 비어 있는 과정은 명시적인 마감 신호가 없으므로 추천에서 유지된다.
- 추천 fallback은 개인화 추천이 비었을 때 섹션 공백을 막기 위한 모집중 공개 후보라, 카드의 `커리어 핏` 점수는 실제 개인화 관련도보다 정렬 점수 성격이 강할 수 있다.
- 대시보드 화면은 이제 공개 fallback 후보를 `내 커리어 핏 추천`에 노출하지 않으므로, 김지원 계정의 개인화 추천이 비었거나 추천 엔진이 실패하면 추천 strip은 빈 상태로 보일 수 있다.
...
- `npx tsc --noEmit --pretty false`: passed after 추천 threshold 제거, 개인화 추천 정렬 유지, 카드 퍼센트 제거.
- `http://localhost:3000/api/dashboard/recommended-programs`: 최초에는 Supabase statement timeout 400을 재현했고, fallback 보강 후 9개 추천/fallback item을 반환함을 확인했다.
