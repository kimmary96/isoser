...
현재 `HEAD`에는 대시보드 캘린더 카드에서 `/dashboard/resume?prefill_program_id=<id>`로 이동하는 CTA가 이미 존재한다. 그러나 `/dashboard/resume`의 실제 편집 상태와 저장 API는 아직 해당 쿼리를 해석하지 않으며, 프로그램 요건 기반 자동 선택/초기값 구성도 없다. 사업계획서가 정의한 순환 플로우("캘린더 → 프로그램 선택 → 요건 분석 → 관련 활동 자동 선택 → 이력서 초안 프리필 → PDF 생성 → 지원 → 다음 마감 캘린더") 중 **출구 단계**인 프리필이 빠져 있어, 사용자가 캘린더에서 관심 프로그램을 발견해도 다시 이력서를 처음부터 구성해야 한다.

이 Task는 다음 흐름을 연결한다:
1. 기존 캘린더 CTA 또는 프로그램 진입점에서 `/dashboard/resume?prefill_program_id=<id>`로 이동
2. 페이지 로드 시 프로그램 요건(직무/스킬/키워드)을 분석하고, 사용자 활동 중 관련도가 높은 활동을 자동 선택
3. `target_job`, `selected_activity_ids`, `summary` 초안을 프리필한 상태로 편집 UI 진입
4. 프리필 상태임을 알리는 배너 + "프리필 되돌리기" 버튼 제공 (저장 전까지 사용자는 자유롭게 조정)

# Current Baseline Validated At HEAD

- `frontend/app/dashboard/_components/dashboard-calendar-section.tsx`는 이미 `/dashboard/resume?prefill_program_id=<id>` CTA를 렌더링한다.
- `frontend/app/dashboard/resume/page.tsx`와 `frontend/app/dashboard/resume/_hooks/use-resume-builder.ts`는 아직 `prefill_program_id`를 읽지 않고, 기본 수동 편집 흐름만 제공한다.
- `frontend/app/api/dashboard/resume/route.ts`는 현재 일반 builder 데이터 조회와 이력서 생성만 지원하며, prefill 조회 route나 `source_program_id` 저장은 없다.
- 따라서 구현 범위는 **캘린더 CTA 추가가 아니라 resume 진입 이후의 프리필 계산, 상태 반영, 저장 payload 확장**이다.
...
- 의존 task인 TASK-2026-04-17-1500과 1510의 결과는 현재 `HEAD`에 반영되어 있으며, 본 task는 그 위에서 resume 프리필 구간만 이어 붙이면 된다
...
- 현재 packet은 최신 `HEAD` 기준으로 재동기화되었으므로, 다음 watcher run은 이 baseline에서 시작한다
...
3. 매칭 로직을 새 BFF route에서 수행할지 FastAPI로 넘길지 — 현재 packet은 기존 `_program_match_context` 또는 동등한 서버측 overlap 계산 재사용을 우선 가정한다
