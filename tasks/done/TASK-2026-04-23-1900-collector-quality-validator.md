...
- `backend\venv\Scripts\python.exe -m pytest backend/tests/test_collector_quality_validator.py backend/tests/test_work24_kstartup_field_mapping.py backend/tests/test_scheduler_collectors.py -q`
  - Result: `32 passed, 6 warnings`

## Completion Note

The implementation was already committed in `5f6229b Add collector quality dry-run validation` and is present in current `HEAD`. This file was moved out of `tasks/drifted/` to prevent the completed task from remaining in the active drift queue.
