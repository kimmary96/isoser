...
planned_against_commit: 469cd3f06a5e9e73cefddcf7181afa014948de69
...
현재 worktree에는 이 task가 목표로 하는 Tier 3 관련 변경이 이미 일부 존재한다. 다음 watcher run은 `backend/rag/collector/tier3_collectors.py`, `backend/rag/collector/scheduler.py`, `backend/tests/test_tier3_collectors.py`, `backend/tests/test_scheduler_collectors.py`의 현재 상태를 먼저 검토한 뒤, 누락된 구현과 검증만 보완해야 한다. 기존 미완료 변경을 덮어쓰거나 새 파일을 다시 생성하는 방식으로 진행하지 않는다.
...
**파일:** `backend/rag/collector/tier3_collectors.py` (현재 worktree에 파일이 이미 있으므로 해당 파일을 기준으로 보완)
...
**파일:** `backend/rag/collector/tier3_collectors.py` (현재 worktree의 기존 구현을 이어서 보완)
...
1. `tier3_collectors.py`를 별도 파일로 분리할지, 기존 `regional_html_collectors.py`에 편입할지 — 현재 Tier 2 파일 크기와 네이밍 관례 확인 후 판단
2. KISED 사업공고 페이지네이션 존재 여부 — 1차는 1페이지만 수집하고, 추가 페이지 여부를 구현 중에 확인
3. KOBIA `EVENTSCH`, `REPORT` 게시판 추가 수집 — 1차 범위 외, 2차에서 결정
...
- 실행 전 현재 HEAD가 `469cd3f06a5e9e73cefddcf7181afa014948de69`인지 확인한다. 다른 commit이면 drift 검토 후 `planned_against_commit`을 실제 HEAD로 교체하고 영향받는 섹션을 재검토한다.
